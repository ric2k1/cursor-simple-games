...
import { GameState, Position, WinLine, Confetti } from './types';
...
export class GameRenderer {
    ...
    private animateConfetti(): void {
        ...
        for (let i = this.confetti.length - 1; i >= 0; i--) {
            ...
            if (confetti.life > confetti.maxLife || 
                (confetti.y > window.innerHeight + 100 && confetti.bounceCount >= confetti.maxBounces)) {
                this.confetti.splice(i, 1);
            }
        }
    ...
    private drawConfetti(): void {
        ...
        for (const confetti of this.confetti) {
            ...
            // 绘制轨迹
            this.drawConfettiTrailOnContext(tempCtx, confetti);
            ...
            // 绘制彩带形状
            this.drawConfettiShapeOnContext(tempCtx, confetti);
            
            // 闪烁效果
            if (confetti.sparkleEffect && Math.random() > 0.95) {
                this.drawSparkleOnContext(tempCtx, confetti);
            }
    ...
    private drawConfettiTrailOnContext(ctx: CanvasRenderingContext2D, confetti: Confetti): void {
    ...
    private drawConfettiShapeOnContext(ctx: CanvasRenderingContext2D, confetti: Confetti): void {
        ...
        switch (confetti.shape) {
            ...
            case 'star':
                this.drawStarOnContext(ctx, confetti.color, halfSize);
                break;
    ...
    private drawStarOnContext(ctx: CanvasRenderingContext2D, color: string, size: number): void {
        ctx.fillStyle = color;
        ctx.beginPath();
    ...
    private drawSparkleOnContext(ctx: CanvasRenderingContext2D, confetti: Confetti): void {
        ctx.save();
        ctx.globalAlpha = 0.8;
